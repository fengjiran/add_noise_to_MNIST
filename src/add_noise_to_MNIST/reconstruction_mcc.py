...
class dA(object):
    ...
    #    noise = 0.9 * self.theano_rng.normal(size = input.shape, avg = 0, std = 0.1) + \
    #            0.1 * self.theano_rng.normal(size = input.shape, avg = 4, std = 0.1)
    ...
    def get_cost_updates(self, contraction_level, corruption_level, 
                         learning_rate, sigma):
        ...
        self.L_rec = - T.sum(T.exp(-((self.x - z)**2)/(2 * (sigma**2))), axis = 1)
...
def test_dA(learning_rate=0.1, training_epochs=15,
            dataset='mnist.pkl.gz', contraction_level = .1, sigma = 1.3,
...
#     train_set_x = theano.shared(value=train_set_x, name = 'train_set_x', borrow = True)
    ...
    image = Image.fromarray(
        tile_raster_images(X=reconstruction_image,
                           ...
                           tile_spacing=(1, 1)))
    image.save('reconstruction_original_mcc.png')

#     image = Image.fromarray(tile_raster_images(
#         X=da.W.get_value(borrow=True).T,
#         img_shape=(28, 28), tile_shape=(10, 10),
#         tile_spacing=(1, 1)))
#     image.save('gaussian_filters_corruption_70.png')
...
if __name__ == '__main__':
    test_dA(learning_rate=0.1, training_epochs=30,
            contraction_level = .1,sigma = 0.5,
            batch_size=20)